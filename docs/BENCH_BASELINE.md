...
### M=1 ×¨ÓÃ kernel ÊµÑé (Task 2.3 ×ÓÈÎÎñ)

³¢ÊÔÎª lm_head (M=1, N=248320, K=2560) Ğ´×¨ÓÃ kernel(1 warp/block£¬Ê¡µô ROWS_PER_BLOCK-1
¿Õ×ª)¡£½á¹û£º**·´¶øÍË»¯**£¬Ô­Òò£º
- GEMV µ÷ÓÃÁ´ÔÚÃ»ÓĞÏÔÊ½ warmup Ê±×ß cublasSgemm »ØÍËÂ·¾¶£¨cuBLAS ÄÚ²¿ÓÃÁË tensor cores
  ºÍ»ã±à¼¶ÓÅ»¯£©£¬ÊÇµ±Ç°µÄ×îÓÅÂ·¾¶
- ×ÔĞ´ JIT kernel ÔÚ M=1 ³¡¾°ÏÂ launch overhead ºÍ SM Õ¼ÓÃ·´¶ø¸ü¸ß
- ¼Ó warmup ºó JIT Â·¾¶»á±»Ê¹ÓÃ£¬µ« cublasSgemm ÔÚ M=1 µÄ GEMV ÉÏÈÔÓÅÓÚ±¾ÏîÄ¿×ÔĞ´ kernel

**½áÂÛ**£º±¾»á»°²»ÔÙ×·Çó×ÔĞ´ kernel ³¬Ô½ cuBLAS£»×ª¶øÑĞ¾¿ GPU ¼¤»î×¤Áô + CUDA Graphs¡£

### 4B hybrid_gpu ÓÅ»¯×·×Ù (»á»° 2026-09-03)

| commit | ÄÚÈİ | decode tok/s | lm_head |
|--------|------|------------:|--------:|
| 17ab3ca | ¼Ó GEMV Ê±¼ä²ÉÑù | 7.88 | CPU prefetch |
| b3d1b9e | ·Å¿ª INT4 ĞĞÉÏÏŞ | 14 | lm_head resident OK but no warmup |
| c6cf a72 | INT4 lm_head dispatch + bench Å¯»ú | 12.59 | 26-30ms (still CPU) |
| 4cd835f | LRU ÌÔÌ­ | 13 | lm_head W16 path Ê§°Ü (4GiB < 2.4GiB needed) |
| **bb07b03** | **lm_head W16 ÉÏ´« + 7GiB Ô¤Ëã** | **19.8** | 10.7ms GPU |

7.88 (CPU baseline) ¡ú **19.8 (hybrid_gpu)** = **2.5x speedup**¡£

Ê£ÓàÆ¿¾± (per token 68ms: linear=28ms + full=8ms + lm_head=10.7ms + misc=20ms):
- linear_attn: 4 ¸ö GEMV (wqkv/wz/wb/wa Í¬ x) ¡ª 4 ¸ö¶ÀÁ¢ launch + D2H, ÈÚºÏ ¡ú ½ÚÊ¡ ~10ms
- gate+up Í¬ÑùÈÚºÏ ¡ú ½ÚÊ¡ ~5ms
- gated_delta_recurrent CPU (100KB¡Á32 heads state) ¡ú ~5ms GPU kernel
- conv1d causal + rmsnorm: µ±Ç° CPU, ¿ÉÉÏ GPU
- argmax (logits[V] È¡ max) ÈÔ CPU, µ¥Ïß³Ì 1MB ¡Ö 0.5ms

ÏÂÒ»²½Â·Ïß ¡ú ¡İ30 tok/s Ğè fused multi-GEMV + GDN ÉÏ GPU + ¼òµ¥ reduce kernel¡£
